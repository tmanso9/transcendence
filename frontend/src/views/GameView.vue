...
onMounted(async () => {
  ...
  if (gameStore.getSocket()) {
    ...
    game(
      canvasRef.value as any,
      gameStore.getSocket(),
      width,
      height,
      source.value,
    );
    gameStore.getSocket().on("kick", () => {
      router.push("/play");
    });
  } else {
...
function changeStyle() {
  img.value.src =
    img.value.src == `${apiURI.replace("3000", "3001")}/afonsinho.jpeg`
      ? `${apiURI.replace("3000", "3001")}/black.png`
      : `${apiURI.replace("3000", "3001")}/afonsinho.jpeg`;
}
...
<template>
  <div class="div-score mx-auto">
    ...
    <div class="score-display">
      <h2>Score</h2>
      <p>
        {{ useGameStore().score.get("paddle1") }} :
        {{ useGameStore().score.get("paddle2") }}
      </p>
    </div>
    <div>
