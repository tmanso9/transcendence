...
export class GameBackendService {
  ...
  private async updatePlayerStats(
    ...
    if (!player) throw new ForbiddenException('Player does not exist');
    ...
    const updated_achievements = this.getAchievements(
      player.achievements,
      updated_streak,
      total_wins,
  ...
  private getRank(points: number): string {
    if (0 <= points && points < 20) return 'NOOBIE';
    else if (20 <= points && points < 50) return 'PRO';
    else if (50 <= points) return 'GURU';
  }

  // TODO: FINISH IMPLEMENTATION
  private getAchievements(
    old_achievements: any,
  ...
  private getStreakAchievements(win_streak: number): string {
    if (5 <= win_streak && win_streak < 10) return Achievements['ON_FIRE'];
    else if (10 <= win_streak && win_streak < 20)
      return Achievements['UNSTOPPABLE'];
    else if (20 <= win_streak) return Achievements['TRANSCENDENT'];
    else return null;
  }

  private getGamesPlayedAchievement(total_games: number): string {
    if (total_games < 5) return Achievements['NOVICE'];
    ...
    else return Achievements['GRANDMASTER'];
  }

  private getRatioAchievement(win_ratio: number): string {
    if (0.5 <= win_ratio && win_ratio < 0.65) return Achievements['CONSISTENT'];
    else if (0.65 <= win_ratio && win_ratio < 0.85)
      return Achievements['DOMINANT'];
    else if (0.85 <= win_ratio) return Achievements['UNBREAKABLE'];
    else return null;
  }
