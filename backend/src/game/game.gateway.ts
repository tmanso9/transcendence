...
export class gameGateway implements OnGatewayDisconnect, OnGatewayConnection {
  ...
  resetGame(client: any, payload: any) {
    ...
    this.server
      .to(this.rooms.get(client.id))
      .emit(
        'gameState',
        this.games.get(this.rooms.get(client.id))?.getGameState() as any,
      );
  }
  ...
  async createRoom(client: any, payload: any) {
    ...
    this.games
      .get(payload.room)
      .registerPlayer(client.id, this.connectedUsers.get(client.id));
    this.rooms.set(client.id, payload.room);
    this.server
      .to(payload.room)
      .emit(
        'playerList',
        Array.from(this.games.get(payload.room).gameState.values()).map(
          (p) => p.username,
        ) as any,
      );
    await this.prismService.user.update({
      where: { id: this.connectedUsers.get(client.id).id },
      data: { status: 'IN_GAME', gameId: payload.room },
    });
    this.roomNumber++;
    ...
    this.server.emit(
      ...
      Array.from(this.games.keys())
        .map((k) => k)
        .filter((k) => k.includes('room')) as any,
    );
    this.server.emit('roomNumber', this.roomNumber as any);
  }
  ...
  async joinRoom(client: Socket, payload: string) {
    if (this.games.has(payload)) {
      ...
      this.server
        ...
        .emit(
          'playerList',
          Array.from(this.games.get(payload).gameState.values()).map(
            (p) => p.username,
          ) as any,
        );
      this.rooms.set(client.id, payload);
      if (
        this.games.get(payload).gameState.size == 2 &&
        this.games.get(payload).spectators.length == 0
      )
        this.games.get(payload).playG(this.server, payload);
      this.server
        .to(payload)
        .emit('gameState', this.games.get(payload).getGameState() as any);
    }
  }
  ...
  async addPlayer(client: Socket, payload: string) {
    ...
    const prismaUser = await this.prismService.user.findUnique({
        ...
        if (game.gameState.size == 2) {
          if (!game.finished) {
            Array.from(game.gameState.values()).forEach((p) => {
              p == game.gameState.get(client.id)
                ? (p.score = 0)
                : (p.score = 10);
            });
          }
          ...
          this.server.to(room).emit('gaveUpOrDisconnected', {
            ...
            winner: Array.from(game.gameState.values()).find(
              (p) => p != game.gameState.get(client.id),
            ).username,
          });
          if (!game.finished) {
